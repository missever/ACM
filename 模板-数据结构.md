...
__gnu_pbds::priority_queue<node, cmp, pairing_heap_tag> q;
```
...
块大小$B=\sqrt n$, 复杂度$n\sqrt{n}$
简单证明:一共$B$个块，那么就有$B * B$个块区间,对于所有询问$[l,r]$在同一个块区间$[l/B,r/B]$的，最多只有$B$次修改,相邻两个块区间也只有$B$次修改
```
//左端点块排序，再右端点排序
...
```
如果发现加减端点操作中某一个不好维护，复杂度较高，可以考虑回滚莫队，都只做容易维护的那个操作，然后记录回滚日志，每次进行块内回滚
...
可以发现, 这样只有路径上的点访问了奇数次, 采用奇数次加偶数次减即可
带单点修改:
仿照普通莫队的修改加一维即可
...
##### 历史版本和
有区间更新操作 和 求区间累计和(相当于每轮会把当前值a[i]加到sum[i]中,区间求sum) 操作
如果更新操作是区间赋值操作的话:
一般每个节点维护 区间历史和$sum$,区间元素之前和$old$(由pushUp传上来) 区间最新值标记$lz$，$old$的累计次数$t$，$lz$累计和$slz$
update的时候只需要将$lz$赋值即可
pushdown时先传递$t$，因为$t$记录的是$old$的次数，所以如果儿子有$lz$则表示$lz$的累加次数，否则表示儿子的$old$的累加次数

如果可以保证累加操作是对整个区间$[1,n]$操作的话，我们可以考虑每个点维护$at+b$形式函数,$a$表示这个点当前最新值
($t$表示已经执行过的累加次数)
那么区间每个点维护的其实就是$suma,sumb$,一个区间$+x$操作其实就等价于区间内所有点$a+=x,b-=x*t$
这相当于维护两个独立的$a,b$每个都有区间加操作和区间求和操作,简单好实现

```c++
struct node {
    LL sum, t, old; // sum = old * t + slz
    int lz, slz; // slz = sum of lz
 
    void update(int tt, int len) {
        if (lz == 0) {
            sum += old * tt;
            t += tt;
        } else {
...
{
    if (f[t].t) {
        f[t << 1].update(f[t].t, mid - l + 1);
        f[t << 1 | 1].update(f[t].t, r - mid);
        f[t].t = 0;
    }
 
    if (f[t].lz != 0) {
        f[t << 1].sum += f[t].slz * (mid - l + 1);
        f[t << 1].slz += f[t].slz;
        f[t << 1].lz = f[t].lz;
 
        f[t << 1 | 1].sum += f[t].slz * (r - mid);
        f[t << 1 | 1].slz += f[t].slz;
        f[t << 1 | 1].lz = f[t].lz;
...
void pushUp(int t, int l, int r, int mid)
{
    f[t].sum = f[t << 1].sum + f[t << 1 | 1].sum;
    f[t].old = (f[t << 1].lz == 0 ? f[t << 1].old : f[t << 1].lz * (mid - l + 1))
            + (f[t << 1 | 1].lz == 0 ? f[t << 1 | 1].old : f[t << 1 | 1].lz * (r - mid));
}

//累计操作
if (f[t].lz != -1) {
     f[t].sum += (r - l + 1) * f[t].lz;
     f[t].slz += f[t].lz;
} else {
     f[t].t++;
     f[t].sum += f[t].old;
...
##### 区间最值
有区间内所有元素和某个数$x$取最值操作 和 区间求和询问
如果最值操作是取$min$操作 则每个节点维护区间内最大值和次大值和最大值的数的数量，区间和
如果最大值$>=x$则直接返回，若$x>=$次大值，更新最大值为$x$，并更新区间和
否则暴力递归下去
根据势能分析，该做法复杂度为$mlogn$
