...
一般每个节点维护 区间历史和hsum,区间元素之前和sum(由pushUp传上来的) 区间update标记lz，sum累计次数tg，lz标记对每个数产生贡献cont(也就是说区间当前元素和实际为$sum+lz*len$,如果update操作为加的话)

进行区间累加时，$hsum+=sum+lz*len, cont+=lz$（len是区间元素数量）

pushdown时 更新lz然后 $son.hsum+=son.sum*tg+cont*son.len,son.cont+=cont,son.tg+=tg$
