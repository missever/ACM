...
int ff(int x) {
    if(p[x] != x) {
        int w = ff(p[x]);
        val[x] += lz[p[x]];
		//lz[x] += lz[p[x]]; 因为每次更新只有一层,所以可以不需要这句
        p[x] = w;
    }
    return p[x];
}
...
struct node {
    LL w;
    int u, k;
    node(LL _w = 0, int _u = 0, int _k = 0): w(_w), u(_u), k(_k) {};
};
struct cmp {
    bool operator()(node a, node b) {
        if(val[a.k] + a.w == val[b.k] + b.w) return a.u > b.u;
        return val[a.k] + a.w > val[b.k] + b.w;
    }
};
...
LL d[maxn];
LL DMST(int rt, int n) {// 根 点数(从0开始)
    int i, u, v, T = 0;
    LL ret = 0;
    memset(vis, 0, sizeof(vis));
    while(true) {
        ...
        for(i = 0; i < n; i++) {
            if(vis[i] > T) continue;
            while(!q[i].empty() && p[q[i].top().u] == i) q[i].pop();
            if(q[i].empty()) {
                if(i == rt) continue;
                else return -1;
            } else {
        ...
        if(cntnode == 0) break;
        for(i = 0; i < n; i++) {
            if(vis[i] > T && p[p[i]] == p[i]) val[i] -= d[p[i]];
            ff(i);
        }
        for(i = 0; i < n; i++) {
            if(vis[i] > T) continue;
...
int main() {
    ...
    cout << DMST(0, n + 1) << endl;
    return 0;
}


//斐波那契堆优化,复杂度V^2(最好最坏情况一样)
struct node {
    LL w;
    int u;
    node(LL _w = 0, int _u = 0): w(_w), u(_u) {};
};
struct cmp {
    bool operator()(node a, node b) {
...
__gnu_pbds::priority_queue<node, cmp, thin_heap_tag> q[maxn];

int pre[maxn], vis[maxn], id[maxn], p[maxn];
LL d[maxn], val[maxn], edge[maxn][maxn];
vector<int> lz;
...
void funion(int n) {
    int i, m = lz.size(), u = lz[0];
    for(auto e:lz) {
        for(i = 0;i < n; i++) {
            edge[i][e] += val[e];
            if(edge[i][e] < edge[i][u]) edge[i][u] = edge[i][e];
        }
    }
    q[u].clear();
    for(i = 0;i < n; i++) {
        if(ff(i) != u) q[u].push(node(edge[i][u], i));
    }
    val[u] = 0;
}
...
LL DMST(int rt, int n) {
    ...
    if(ret > inf) ret = -1;//如果答案中出现了一条无穷大边,视为无解
	//如果边权太大,无法这样判,就dfs一遍判断是否有解即可
    return ret;
}
...
// 先调用init初始化  然后调用add_edge加边
// 调用solve求最大匹配  pair的first是最大权和 second是匹配数
// match[i]表示和i点匹配的点的编号  等于0表示没有匹配
```
...
long long ans[N];
// idom[x] 最近支配点 即树根到x的路径上距离x最近的必经点(所求都是以root为起点的必经点)
